...
<div id="cvwp-video-settings" class="cvwp">
	...
	<?php else : // theme compatibility not enabled, show embedding video settings ?>
        ...
        <input type="hidden" name="cvwp_post[video][source]" value="<?php echo esc_attr( $options['video']['source'] ); ?>" />
        <input type="hidden" name="cvwp_post[video][video_id]" value="<?php echo esc_attr( $options['video']['video_id'] ); ?>" />
        <input type="hidden" name="cvwp_post[video][duration]" value="<?php echo esc_attr( absint( $options['video']['duration'] ) ); ?>" />
        ...
        <table class="form-table" id="cvwp-video-settings-table">
                ...
                <tr>
                    ...
                    <td>
						<?php if ( cvwp_is_override_set( 'embed_position' ) ) : ?>
                            ...
                            <?php printf(
	                            __( '<strong>Not configurable.</strong> Option can be changed globally from plugin %sSettings%s page.', 'videographywp' ),
	                            '<a href="' . esc_url( menu_page_url( 'cvwp-settings', false ) ) . '">',
	                            '</a>'
                            ); ?>
                        </span>
						<?php else : ?>
							...
							?>
                            <span class="description"><?php _e( 'Choose where to embed the video into the post content.', 'videographywp' ); ?></span>
						<?php endif; ?>

						<?php $cls = ( 'shortcode' === $options['embed_position'] ) ? '' : 'hide-if-js '; ?>
                        <p class="<?php echo esc_attr( $cls ); ?>cvwp-note" id="cvwp-embed-position-notice">
                        ...
                        <p class="<?php echo esc_attr( $cls ); ?>cvwp-note" id="cvwp-embed-button-notice">
						...
						?>
                        <p class="<?php echo esc_attr( $cls ); ?>cvwp-warning" id="cvwp-no-embed-notice">
                            <span class="description"><?php _e( '<strong>Please note</strong>: Video won\'t be embedded by the plugin.', 'videographywp' ); ?></span>
                        </p>

						<?php $cls = ( 'featured_image' === $options['embed_position'] ) ? '' : 'hide-if-js '; ?>
                        <p class="<?php echo esc_attr( $cls ); ?>cvwp-note" id="cvwp-featured-image-position-notice">
                            <span class="description"><?php _e( "It's essential to have a featured image selected for this post; without it, the video cannot be embedded.", 'videographywp' ); ?></span>
                        </p>
			...
			if ( $has_lazy ) :
				?>
                <tr class="<?php echo esc_attr( $noembed_cls ); ?>cvwp-no-embed-hide">
			...
			if ( $has_size ) :
                ...
                <tr class="<?php echo esc_attr( $noembed_cls ); ?>cvwp-no-embed-hide">
                    ...
                    <td class="cvwp-player-aspect-options">
						...
						} elseif ( has_filter( 'cvwp_embed_width' ) ) {
							$disabled = ' disabled="disabled"';
						}
						?>
                        <label for="cvwp-width"><?php _e( 'Width', 'videographywp' ); ?>:</label>
                        <input
                                class="cvwp_video_width small-text"
                                size="2"
                                type="number"
                                step="5"
                                min="0"
                                id="cvwp-width"
                                name="cvwp_post[video][width]"
                                value="<?php echo esc_attr( $width ); ?>"<?php echo $disabled; ?>
                        /> px |
						<?php _e( 'Height', 'videographywp' ); ?>:
                        <span class="cvwp_video_height">
                        <?php echo esc_html( cvwp_player_height( $options['video']['aspect'], $width ) ); ?>
                    </span> px
						<?php if ( cvwp_is_override_set( 'video_width' ) && ! has_filter( 'cvwp_embed_width' ) ) : ?>
                            <p class="description no-config">
								<?php printf(
									__( '<strong>Width is not configurable.</strong> Option can be changed globally from plugin %sSettings%s page.', 'videographywp' ),
									'<a href="' . esc_url( menu_page_url( 'cvwp-settings', false ) ) . '">',
									'</a>'
								); ?>
                            </p>
                    ...
                    <td>
						...
						<?php else : ?>
                            <input
                                    ...
                                    step="5"
                                    min="0"
                                    value="<?php echo esc_attr( absint( $options['video']['volume'] ) ); ?>"
                                    name="cvwp_post[video][volume]"
                                    id="cvwp-volume"
                                    size="2"
                            />
                ...
                <tr class="<?php echo esc_attr( $noembed_cls ); ?>cvwp-no-embed-hide">
                    ...
                    <td>
						<?php if ( cvwp_is_override_set( 'video_loop' ) ) : ?>
                            <span class="description no-config">
                            <?php printf(
	                            __( '<strong>Not configurable.</strong> Option can be changed globally from plugin %sSettings%s page.', 'cvwp' ),
	                            '<a href="' . esc_url( menu_page_url( 'cvwp-settings', false ) ) . '">',
	                            '</a>'
                            ); ?>
                        </span>
	...
	<?php endif; // endif theme compatibility enabled ?>
        ...
        <div class="<?php echo esc_attr( $noembed_cls ); ?>cvwp-no-embed-hide">
                    ...
                    <table class="form_table">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <span class="description"><?php _e( 'when checked will show logo on video', 'cvwp' ); ?></span>
                            </td>
                        </tr>
                        </tbody>
                    </table>
